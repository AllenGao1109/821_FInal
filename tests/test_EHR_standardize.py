...
from src.EHR_processing_functions import EHR_standardize
...
def test_EHR_standarize_basic(tmp_path: Any) -> pd.DataFrame:
    ...
    result = EHR_standardize(
        dataset=str(csv_file), potential_missing=[""], format="csv"
    )
...
def test_EHR_standarize_wrong_file_format(tmp_path: Any) -> None:
    ...
    with pytest.raises(
        ValueError,
        match=r"File extension '.csv' does not match "
        "expected format: 'tsv'",
    ):
...
def test_EHR_standarize_empty_file(tmp_path: Any) -> None:
    ...
    with pytest.raises(ValueError, match=r".*data\.csv is empty"):
...
def test_EHR_standardize_multiple_missing_values(tmp_path: Any) -> None:
    ...
    result = EHR_standardize(
        dataset=str(csv_file), potential_missing=missing_values, format="csv"
    )
