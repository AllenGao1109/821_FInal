...
import pandas as pd
import pytest
from EHR_processing_functions import EHR_filter_cols
...
def test_EHR_filter_cols_missing_columns() -> None:
    ...
    expected_error = re.escape("Columns not found in DataFrame:"
    " ['missing_col']")
...
def test_EHR_filter_cols_empty_values() -> None:
    ...
    with pytest.raises(ValueError, match="At least one value to remove " \
    "must be specified"):
...
def test_EHR_filter_cols_all_rows_removed() -> None:
    ...
    with pytest.raises(ValueError, match="Filtering would remove all rows " \
    "from the DataFrame"):
