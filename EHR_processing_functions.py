...
def EHR_standardize(
    ...
    if (
        not isinstance(potential_missing, list) or 
        not all(isinstance(x, (str, int, float)) for x in potential_missing)
    ):
        raise ValueError(f"{potential_missing}"
                         f"is not a list of strings, integers or floats")
...
def EHR_preprocessing(df:pd.DataFrame, 
    ...
    if remove:
        ...
        if df.empty:
            raise ValueError(
                "DataFrame is empty: " \
                "At least one column is all np.nan"
                )
...
def EHR_filter_cols(df: pd.DataFrame,
    variables: list[str],
    values_to_remove: Sequence[Union[str, int, float, bool]]) -> pd.DataFrame:
